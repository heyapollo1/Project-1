...
public class PlayerAbilityManager : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        EventManager.Instance.StartListening("EnablePlayerAbilities", EnableAbilities);
    ...
    {
        ResetAbilities();
        EventManager.Instance.StopListening("EnablePlayerAbilities", EnableAbilities);
    ...
    public List<string> GetAbilityNames()
    {
        ...
        foreach (var ability in playerAbilities)
        {
            if (ability != null && ability.abilityData != null)
            {
                abilityNames.Add(ability.abilityData.abilityTitle);
            }
        }
        return abilityNames;
    }
    
    public void LoadAbilitiesFromSave(PlayerState playerState)
    {
        ResetAbilities();

        foreach (string abilityName in playerState.acquiredAbilities)
        {
            var abilityData = PlayerAbilityDatabase.Instance.GetAbilityDataByName(abilityName);
            if (abilityData != null)
            {
                UnlockAbility(abilityData, transform);
            }
            else
            {
        ...
        Debug.Log($"Loaded {playerState.acquiredAbilities.Count} abilities from save.");
    }
}
